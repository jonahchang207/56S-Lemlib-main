...
// Auton names matching the order in main.cpp switch statement
...
static const int auton_count = 4;
...
static void btn_event_cb(lv_event_t* e) {
    ...
    if (auton_options != nullptr) {
        uint32_t count = lv_obj_get_child_count(auton_options);
        for (uint32_t i = 0; i < count; i++) {
            lv_obj_t* child = lv_obj_get_child(auton_options, i);
            if (!child) continue;
    ...
    if (auton_desc_label) {
        if (btn_id >= 0 && btn_id < auton_count) {
            lv_label_set_text(auton_desc_label, auton_names[btn_id]);
        } else {
...
void update_match_data() {
    ...
    if (selected_auton >= 0 && selected_auton < auton_count) {
